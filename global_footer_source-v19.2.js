...
function setChannelStates(permissions, cachedResponse) {
    ...
    if (path != kReleaseToProductionPath && !user?.role.includes("Ori Read-only")) {
    ...
    if (!cachedResponse.hasReleasedToProduction && !user?.role.includes("Ori Read-only")) {
        setChannelLocked(kInstallAttribute);
        setChannelLocked(kLeasingAttribute);
        setChannelLocked(kManagementAttribute);

        if (path == kInstallAttribute || path == kLeasingAttribute || path == kManagementAttribute) {
...
function configureChannel(channelAttribute, permissions) {
    console.log("configuring channel:", channelAttribute, permissions);
