...
const kChannelAttributes = [
    ...
    // kReleaseForShipmentAttribute, // TODO: KADAR - add back after support
    kLeasingAttribute,
...
const kPreInstallPath = "/pre-install"; // Missing Items
const kInstallPath = "/install";
const kLeasingPath = "/leasing";
const kManagementPath = "/management";
const kLockInPricePath = "/lock-in-price";
const kReleaseToProductionPath = "/release-to-production";
const kReleaseForShipmentPath = "/release-for-shipment";
...
async function populateLefthandNavigation(cachedResponse) {
    ...
    if (!cachedResponse) {
        setChannelStates([], cachedResponse);
        return;
    }
    ...
    setChannelStates(permissions, cachedResponse);
}
...
function setMissingItemsNotificationState(count) {
    const missingItemsBadge = document.querySelector(`[missing-items="badge"]`);
    const missingItemsBadgeCount = document.querySelector(`[missing-items="badge-number"]`);

    if (count > 0) {
        if (missingItemsBadge) {
            missingItemsBadge.style.display = "flex";
        }
        if (missingItemsBadgeCount) {
            missingItemsBadgeCount.style.display = "flex";
            missingItemsBadgeCount.innerText = count;
        }
    } else if (count > 9) {
        if (missingItemsBadge) {
            missingItemsBadge.style.display = "flex";
        ...
        if (missingItemsBadgeCount) {
            missingItemsBadgeCount.style.display = "none";
        }
    } else {
        if (missingItemsBadge) {
            missingItemsBadge.style.display = "none";
        }
        if (missingItemsBadgeCount) {
            missingItemsBadgeCount.style.display = "none";
        }
...
function setChannelStates(permissions, cachedResponse) {
    ...
    if (path != kReleaseToProductionPath) {
        if (cachedResponse.releaseToProductionAvailable) {
            setChannelInactive(kReleaseToProductionAttribute);
        } else {
            setChannelLocked(kReleaseToProductionAttribute);
        }
    }

    if (!cachedResponse.hasReleasedToProduction) {
        console.log("Has not released to production, locking install, leasing, and management.");
        setChannelLocked(kInstallAttribute);
        setChannelLocked(kLeasingAttribute);
        setChannelLocked(kManagementAttribute);

        if (!isMobileDevice() && (path == kInstallPath || path == kLeasingPath || path == kManagementPath)) {
            window.location = window.location.origin + kIntroPath;
        }
    }
}

function setChannelLocked(channelAttribute) {
    setChannelVisible(channelAttribute, kLockedState);
    setChannelHidden(channelAttribute, kActiveState);
    setChannelHidden(channelAttribute, kInactiveState);
}

function setChannelInactive(channelAttribute) {
    setChannelVisible(channelAttribute, kInactiveState);
    setChannelHidden(channelAttribute, kActiveState);
    setChannelHidden(channelAttribute, kLockedState);
}

function setChannelActive(channelAttribute) {
...
function configureChannel(channelAttribute, permissions) {
    setChannelActive(channelAttribute);
    ...
    for (const attribute of kChannelAttributes) {
        if (attribute != channelAttribute) {
            ...
            console.log("configuring channel channelPermissions", channelPermissions);
            if (attribute == kIntroAttribute || hasChannelPermission(permissions, channelPermissions)) {
                setChannelInactive(attribute);
            } else {
                setChannelLocked(attribute);
            }
        }
    }
}
...
function hasChannelPermission(permissions, channelPermissions) {
    console.log("hasChannelPermissions permissions:", permissions);
    console.log("hasChannelPermissions channelPermissions:", channelPermissions);
...
const login = async () => {
    ...
    try {
        await auth0Client.loginWithRedirect({
            authorizationParams: {
                redirect_uri: window.location.origin + kIntroPath
            }
        });
    } catch (error) {
...
const handleAuth0 = async () => {
    ...
    console.log("missingItemsContent", missingItemsContent);
    ...
    if (logoutButtons && isIntroPage) {
        if (isAuthenticated) {
            ...
            if (missingItemsContent) {
                console.log("showing missing items content");
            ...
            if (missingItemsContent) {
                console.log("hiding missing items content");
                missingItemsContent.style.display = "none";
            }
        }
    }
...
async function getPortalResponse() {
    ...
    // Ignore cachedResponse to get Missing Items and Release To Production states
